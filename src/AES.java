...
public class AES {
	...
	public static void main(String args[]) throws IOException {
		...
		try {
			...
			key = convertStringToHex(keyreader.readLine());
		...
		if (args[0].equalsIgnoreCase("e")) {
			...
			int numRounds = 10;
			String line = convertStringToHex(input.readLine()).toUpperCase();
			...
			while (line != null) {
				if (line.matches("[0-9A-F]+")) //If line is valid (i.e. contains valid hex characters, encrpyt. Otherwise, skip line.
				{
					...
					{
						for (int j = 0; j < 4; j++) {
							state[j][i] = StringToMatrix(line, i, j);
						}
					}

					aes.addRoundKey(state, aes.subKey(keymatrix, 0)); //Запускает addRoundKey с первой частью расширения ключа
					for (int i = 1; i < numRounds; i++) {
						aes.subBytes(state); //реализует метод Sub-Bytes.
						aes.shiftRows(state); //реализует метод Shift-Rows.
						aes.mixColumns(state); //реализует метод Mix-Columns
						aes.addRoundKey(state, aes.subKey(keymatrix, i)); //Запускает addRoundKey
					}
					aes.subBytes(state); //реализует метод Sub-Bytes.
					aes.shiftRows(state);  //реализует метод Shift-Rows.
					aes.addRoundKey(state, aes.subKey(keymatrix, numRounds)); //Запускает addRoundKey
		...
		} else if (args[0].equalsIgnoreCase("d")) //Decryption Mode
		{
			...
			while (line != null) {
				...
				out.write(hex_to_ascii(MatrixToString(state) + newline));
				line = input.readLine();
			}
			input.close();
			out.close();
	...
	private static int StringToMatrix(String line, int i, int j) {
		return Integer.parseInt(line.substring((8 * i) + (2 * j), (8 * i) + (2 * j + 2)), 16);
	}
	...
	public int[][] keySchedule(String key) {
		...
		int colsize = binkeysize + 48 - (32 * ((binkeysize / 64)
				- 2)); //size of key scheduling will be based on the binary size of the key.
		...
		for (int i = 0; i < keycounter;
				i++) //the first 1 (128-bit key) or 2 (256-bit key) set(s) of 4x4 matrices are filled with the key.
		{
			for (int j = 0; j < 4; j++) {
	...
	/**
	 * На этапе AddRoundKey подраздел объединяется с состоянием. Для каждого раунда извлекается часть запланированного ключа
	 * Каждый подраздел имеет тот же размер, что и состояние.
	 * Каждый элемент в байтовой матрице подвергается операции XOR с каждым элементом в фрагменте расширенного ключа.
	 *
